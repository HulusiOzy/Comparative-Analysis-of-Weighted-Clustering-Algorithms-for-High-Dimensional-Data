...
class WKMeans:
    def __init__(self, filename='iris.data', k=3, beta=-1, metric='p0'):
        self.filename = filename
        self.k = k
        self.beta = beta
        self.metric = metric.lower()  #P or S(i)
        self.labels_ = None
        self.weights_ = None
    ...
    #If I had a brain in my head I couldve done this in 1 function but I dont so...
    ...
    def _calculate_silhouette(self, labels):
        ...
        os.remove(temp_pred_file)  #Clean up temporary file so I dont have dumb duplicates
        return score
    ...
    def _P0(self, data_points, U, Z, W):
        ...
        total_error = 0
        W_beta = W**self.beta #So I dont repeat this every single iteration
        ...
        if h > 0: #Only proceed if we have any non zero Dj. Does have scenarios, alot actually. Trust me of the past on this one.
            for j in range(N):
                if Dj[j] != 0: #One of these checks are redundant but whatever
                    ...
                    #Sum Dj/Dt ^ 1/(beta-1)
                    sum_term = np.sum([(Dj[j]/Dj[t])**(1/(self.beta-1)) 
                            for t in range(N) 
    ...
    def _run_wkmeans(self, data_points):
        M, N = data_points.shape
        
        indices = np.random.choice(M, self.k, replace=False) #Pick rand points by row
        ...
        U = self._P1(data_points, Z, W) #For Step 1
    ...
    def fit(self):
        ...
        self.labels_, self.weights_, self.centroids_, self.partition_matrix_, p0_value = self._run_wkmeans(self.data)
...
def method1():
    ...
    for i in range(11, 50):
        ...
        best_labels = None
        for _ in range(50):
            wkmeans = WKMeans(filename=input_filename, k=3, beta=beta, metric='p0')
            labels, p0 = wkmeans.fit()
    ...
    for i, beta in enumerate(betas):
        wkmeans = WKMeans(filename=input_filename, k=3, beta=beta, metric='si')
        si_values[i] = wkmeans._calculate_silhouette(best_runs[beta])
